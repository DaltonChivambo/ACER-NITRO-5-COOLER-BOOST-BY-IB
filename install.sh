...
[ -f "$SCRIPT_DIR/assets/nitro-boost-ib.svg" ] && cp "$SCRIPT_DIR/assets/nitro-boost-ib.svg" "$INSTALL_DIR/" && \
  mkdir -p /usr/share/icons/hicolor/scalable/apps && \
  cp "$SCRIPT_DIR/assets/nitro-boost-ib.svg" /usr/share/icons/hicolor/scalable/apps/nitro-boost-ib.svg && \
  gtk-update-icon-cache -f /usr/share/icons/hicolor 2>/dev/null || true
[ -f "$SCRIPT_DIR/assets/nitro-boost-ib.png" ] && cp "$SCRIPT_DIR/assets/nitro-boost-ib.png" "$INSTALL_DIR/"
chmod +x "$INSTALL_DIR/nitro-boost"
ln -sf "$INSTALL_DIR/nitro-boost" /usr/local/bin/nitro-boost

if [ -f "$SCRIPT_DIR/config/nitro-boost-ib.desktop" ]; then
  sed "s|/usr/local/lib/nitro-boost|$INSTALL_DIR|g" "$SCRIPT_DIR/config/nitro-boost-ib.desktop" > /tmp/nitro-boost-ib.desktop
...
if [ -f "$SCRIPT_DIR/config/nitro-boost.polkit.rules" ]; then
  mkdir -p /etc/polkit-1/rules.d
  cp "$SCRIPT_DIR/config/nitro-boost.polkit.rules" /etc/polkit-1/rules.d/50-nitro-boost.rules
...
if [ -f "$SCRIPT_DIR/config/nitro-boost.sudoers" ]; then
  cp "$SCRIPT_DIR/config/nitro-boost.sudoers" /etc/sudoers.d/nitro-boost
